...
const SecondaryImageSection = () => {
  ...
  // Define images and corresponding tags
  const images = [woman, woman2, woman3];
  const tags = [t("s8-text1"), t("s8-text2"), t("s8-text3")];

  // State to keep track of current image and tag
  const [currentIndex, setCurrentIndex] = useState(0);

  useEffect(() => {
    const intervalId = setInterval(() => {
      // Update the index for both image and tag
      setCurrentIndex((prevIndex) => (prevIndex + 1) % images.length);
    }, 2000); // 2 seconds interval for demonstration
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: `url(${images[currentIndex]})`, // Set the background image dynamically
      }}
    >
        ...
        <div className="Frame86 p-2.5 left-[10%] top-[411px] absolute bg-white/20 rounded-[280px] backdrop-blur-[14px] flex-col justify-center items-center gap-2.5 inline-flex">
          <div className="Frame85 w-full m-auto justify-between items-center inline-flex">
            <div className="Frame84 justify-start items-center flex gap-3">
              ...
              <div className="text-wrap w-[150px] md:w-[272px] xxx text-right text-white text-xs font-normal font-['Segoe UI'] leading-none">
                {tags[currentIndex]} {/* Display the corresponding tag */}
              </div>
            </div>
          </div>
        </div>
